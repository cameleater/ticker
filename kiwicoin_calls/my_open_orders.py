...
Message = str(Nonce) + str(UserId) + Key + ";open_orders"
Signature = hmac.new(Secret, msg=Message, digestmod=hashlib.sha256).hexdigest().upper()
R = requests.post("https://kiwi-coin.com/api/open_orders/", data={'key': Key, 'signature': Signature, 'nonce': Nonce, 'timeframe': "all"})
